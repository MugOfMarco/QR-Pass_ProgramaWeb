class RegistroSystem {
    ...
    async handleBoletaInput(boletaInput, tipoEntrada) {
        ...
        try {
            ...
            if (data.success) {
                if (data.bloqueado) {
                    this.mostrarError('CREDENCIAL BLOQUEADA');
    ...
    async verificarRetardoConHorario(horario) {
        ...
        const diaActual = diaActualConAcentos.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    mostrarResultadoFinSemana(alumnoData, puerta, tipo) {
        ...
        setTimeout(() => { document.getElementById('boleta-input').value = ''; }, 2000);
    }
    ...
    obtenerTipoRegistroTexto(idTipoRegistro, tieneRetardo, sinCredencial) {
        const tipos = {0: 'entrada', 1: 'salida', 2: 'retardo', 3: 'entrada_sin_credencial', 4: 'justificado'};
    ...
    formatearNombrePuerta(puertaValue) {
        const puertas = {'mexico-tacuba': 'México-Tacuba', 'mar': 'Mar-Mediterráneo'};
